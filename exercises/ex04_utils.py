...
def all(given_list: list[int], given_int: int) -> bool:
    ...
    if len(given_list) <= 0:  # check if list isn't empty
        return False
...
def max(input: list[int]) -> int:
    ...
    while idx < len(input):  # check the item at each index in the list
...
def is_equal(first_list: list[int], second_list: list[int]) -> bool:
    ...
    if len(first_list) < 0:  # check for not an empty list
        return False
    if len(second_list) < 0:  # check for not an empty list
        return False 
    if len(first_list) != len(second_list):  # check that both lists are the same length
        return False
    else:
